...
salmonid_extents <- read_sf('../data-raw/rearing_spatial_data/salmonid_habitat_extents/salmonid_habitat_extents.shp') %>%
  st_transform(crs = "+proj=longlat +datum=WGS84 +no_defs")

# add north and south deltas to rearing extent to spring run 
delta_subset <- salmonid_extents %>%
  filter(River %in% c("North Delta", "South Delta")) %>% 
  mutate(Species = "Spring Run Chinook") 

salmonid_extents <- bind_rows(delta_subset, salmonid_extents)
```
